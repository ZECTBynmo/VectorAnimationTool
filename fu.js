...
fu.staticDirHandler = function( dirname, isRecursive, isReadOnly, fileNames ) {
	// Default to searching recursively
	if( typeof isRecursive == "undefined" ) {
		isRecursive = true;
	}
	
	// Default to both read and send files to the client
	...
	readdir(dirname, function(err,files) {
		if (err) {
			...
			for (x = 0; x < files.length; ++x) {
				if( isRecursive && files[x].lastIndexOf(".") == -1 ) {
					// This is a folder, call into it recursively
					fu.staticDirHandler( dirname + files[x] + "/" );
				} else {
					// Send this file to the client
					fu.get("/" + dirname + files[x], fu.staticHandler(dirname + files[x]));
					
					// Record that we found this file
					fileNames.push( dirname + files[x] );
				}
			} // end for each file
		}
	}); // end readdir()
}; // end staticDirHandler()


//////////////////////////////////////////////////////////////////////////
// Our mime data lookup
fu.mime = {
